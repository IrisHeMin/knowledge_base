---
layout: post
title: "DNS 解析 app.powerbi.com 间歇性失败 — 本地 Zone 劫持 CNAME 链"
date: 2026-02-03
categories: [DNS, Windows-Server]
tags: [dns, zone, cname, powerbi, azurewebsites, forwarder, ttl]
case_number: "2602030030000451"
---
# Case Summary: DNS 解析 app.powerbi.com 间歇性失败 — 本地 Zone 劫持 CNAME 链

**Case Number:** 2602030030000451
**Date:** 2026-02-03
...
- 问题**间歇性发生**：时而正常，时而失败
- 如果在客户端手动指定 DNS forwarder IP（`114.114.115.115` 或 `114.114.114.110`）进行查询，则**始终能正常解析**

## 2. 背景 (Background / Environment)

- **DNS Server:** `chnsh-utildc1.sembutil.corp`（IP: `172.19.1.221`），同时作为 Domain Controller
- **客户端 IP:** `172.19.2.205`
- **DNS Forwarder 配置:** `114.114.115.115`、`114.114.114.110`
...
- `app.powerbi.com` 的完整 CNAME 解析链：
  ...
  app.powerbi.com
    ...
    → 997de1ee-c405-4364-8b90-eb6f601a6af2.trafficmanager.net  (CNAME, TTL: 3628s)
    → app-pbi-wfe-taiwan-north-v3.pbi-wfe-taiwan-north-v3-ase.p.azurewebsites.net  (CNAME, TTL: 300s)
...
1. **在 DNS Server 上抓包，从客户端发起查询**
   - **做了什么：** 在 DNS Server `chnsh-utildc1.sembutil.corp` 上启动网络抓包（Network Monitor），同时从客户端 `172.19.2.205` 发起对 `app.powerbi.com` 的 DNS A 记录查询
   - **发现了什么：**
     ...
     - Frame 7295（10:52:27）：DNS Server 返回响应，状态码 Success，包含 **3 条 CNAME 记录**，但**没有最终的 A 记录（无 IP）**：
       ...
       app.privatelink.analysis.windows.net → 997de1ee-...trafficmanager.net (CNAME)
       997de1ee-...trafficmanager.net → app-pbi-wfe-...-v3.p.azurewebsites.net (CNAME)
       ```
...
2. **检查 DNS Server 上的 Zone 配置**
   ...
   - **得出结论：** 当 DNS Server 尝试解析 `app-pbi-wfe-...-v3.p.azurewebsites.net` 时，命中了本地 Zone `azurewebsites.net`。由于 DNS Server 认为自己是该 Zone 的权威服务器（authoritative），它**不会再将查询转发给 forwarder**。而本地 Zone 中无对应 A 记录，因此返回空结果

3. **验证直接使用 Forwarder 可以正常解析**
   - **做了什么：** 在客户端直接指定 forwarder `114.114.115.115` / `114.114.114.110` 进行查询
...
- **方案 B：** 在 Zone `azurewebsites.net` 中手动添加 `app-pbi-wfe-taiwan-north-v3.pbi-wfe-taiwan-north-v3-ase.p` 的 A 记录 → **不推荐**，因为 Azure 的 IP 会变动，维护成本高
- **方案 C：** 在 Zone `azurewebsites.net` 下创建 Delegation `p.azurewebsites.net`，将 NS 指向 forwarder（`114.114.115.115`、`114.114.114.110`）→ 当解析 `xxx.p.azurewebsites.net` 时会命中此 delegation，查询被正确转发
...
# Case Summary: Intermittent DNS Resolution Failure for app.powerbi.com — Local Zone Hijacking CNAME Chain
...
- Querying directly against DNS forwarders (`114.114.115.115` or `114.114.114.110`) **always succeeds**

## 2. Background / Environment

- **DNS Server:** `chnsh-utildc1.sembutil.corp` (IP: `172.19.1.221`), also serving as Domain Controller
- **Client IP:** `172.19.2.205`
- **DNS Forwarders:** `114.114.115.115`, `114.114.114.110`
...
1. **Captured network trace on DNS Server while querying from the client**
   - **Action:** Started a network capture (Network Monitor) on DNS Server `chnsh-utildc1.sembutil.corp`; initiated a DNS A record query for `app.powerbi.com` from client `172.19.2.205`
   - **Finding:**
...
2. **Examined DNS Server zone configuration**
   ...
   - **Conclusion:** When the DNS Server attempted to resolve `app-pbi-wfe-...-v3.p.azurewebsites.net`, it matched the local zone `azurewebsites.net` and considered itself authoritative — therefore it did **not** forward the query to forwarders. Since the zone had no matching A record, it returned no IP

3. **Verified resolution works when pointing directly to forwarders**
   - **Action:** Queried `app.powerbi.com` directly against forwarders `114.114.115.115` / `114.114.114.110`
...
- **Option B:** Add a static A record for `app-pbi-wfe-taiwan-north-v3...p` in the zone → **Not recommended** — Azure IPs change frequently, high maintenance burden
- **Option C:** Create a delegation `p.azurewebsites.net` under the zone, with NS records pointing to the forwarders (`114.114.115.115`, `114.114.114.110`) → queries for `*.p.azurewebsites.net` will hit the delegation and be properly forwarded

## 6. Lessons Learned

- **Technical Knowledge:**
  - When a DNS Server hosts a local zone, **all queries for names under that zone are resolved locally** — they are never forwarded, even if the zone has no matching records. This is standard DNS authoritative behavior: once a server considers itself the authority for a zone, it will not query externally
